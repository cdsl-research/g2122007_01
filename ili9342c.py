...
class ILI934X:
    ...
    >>> display = ili9342c.ILI934X(spi, cs=Pin(0), dc=Pin(5), rst=Pin(4))
    >>> display.fill(ili9342c.color565(0xff, 0x11, 0x22))
    >>> display.pixel(120, 160, 0)
    """

    width = 320 # mod:ILI9342C on M5Stack
    height = 240 # mod:ILI9342C on M5Stack

    def __init__(self, spi, cs, dc, rst, bl): # add:ILI9342C on M5Stack
        self.spi = spi
        self.cs = cs
        self.dc = dc
        self.rst = rst
        self.bl = bl # add:ILI9342C on M5Stack
        self.cs.init(self.cs.OUT, value=1)
        self.dc.init(self.dc.OUT, value=0)
        self.rst.init(self.rst.OUT, value=0)
        self.bl.init(self.bl.OUT, value=0) # add:ILI9342C on M5Stack
        self.reset()
        self.init()
        self._scroll = 0
    ...
    def init(self):
        ...
        self._write(_MADCTL, b'\x08') # add:ILI9342C on M5Stack
        self._write(_DISPLAY_INVERSION_ON) # add:ILI9342C on M5Stack
        self._write(_WAKE)
        time.sleep_ms(120)
        self._write(_DISPLAY_ON)
        self.bl.value(1) # add:ILI9342C on M5Stack

    def reset(self):
        self.bl.value(0) # add:ILI9342C on M5Stack
        self.rst.value(0)
        time.sleep_ms(50)
        self.rst.value(1)
        time.sleep_ms(50)
